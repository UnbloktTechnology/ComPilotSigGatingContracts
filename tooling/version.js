...
const tsFilePath = path.join(__dirname, '../sig-gating-contracts/deploy/01_deployNexeraIDSignerManager.ts');
...
tsFileContent = tsFileContent.replace(/const version = "(\d+)\.(\d+)\.(\d+)"/, (match, major, minor, patch) => {
