...
contract TxAuthDataVerifierFacet is Context {
    ...
    function _verifyTxAuthData(
        ...
        bytes calldata argsWithSelector = msgData[:msgData.length -
            TxAuthDataVerifierFacetStorage._EXTRA_DATA_LENGTH];

        uint256 blockExpiration = uint256(
            bytes32(
                msgData[msgData.length -
                    TxAuthDataVerifierFacetStorage._EXTRA_DATA_LENGTH:msgData
                    .length - TxAuthDataVerifierFacetStorage._SIGNATURE_LENGTH]
            )
        );

        bytes calldata signature = msgData[msgData.length -
            TxAuthDataVerifierFacetStorage._SIGNATURE_LENGTH:];
