...
import { signTxAuthDataLib } from "@compilot/sig-gating-contracts-sdk/lib";
...
describe(`CompilotSignerManager`, function () {
	...
	beforeEach(async () => {
		...
		({ exampleGatedNFTMinter, compilotIDSignerManager } =
	...
	it(`Should check that signerManagerControllerSigner can change the signer`, async () => {
		...
		// set signer
		await compilotIDSignerManager
			.connect(signerManagerControllerSigner)
			.setSigner(tester2);

		const newSigner = await compilotIDSignerManager.signerAddress();
	...
	it(`Should check that non-signerManagerControllerSigner can NOT change the signer`, async () => {
		...
		// try to set signer
		await expect(
			compilotIDSignerManager.connect(tester2Signer).setSigner(tester2),
