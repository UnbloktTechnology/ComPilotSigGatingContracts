...
import { TezosTxAuthData, TezosTxCalldata } from "../../src/lib/schemas";
...
//TODO derive userAddress from userPublicKey
function compute_payload_hash_for_mint(
  ...
  const key_bytes = convert_key(userPublicKey);
  const payload =
    key_bytes +
    chain_id_bytes +
    user_bytes +
...
describe(`Tezos ExampleGatedNFTMinter`, function () {
  ...
  it(`Should mint the asset #1`, async () => {
    ...
    const contractAddress = exampleGatedNFTMinter ? exampleGatedNFTMinter : "";
    ...
    const userPublicKey =
      "edpkurPsQ8eUApnLUJ9ZPDvu98E8VNj4KtJa1aZr16Cr5ow5VHKnz4"; // bob public key
    const expiration = (currentBlock + 10).toString();
    const nonce = "0";
    const userAddress = "tz1fon1Hp3eRff17X82Y3Hc2xyokz33MavFF";
    const chainID = currentChainId;

    //TODO: fetch current block and add enough for 5mn
    const blockExpiration = 1000;
    ...
    const payload_hash = compute_payload_hash_for_mint(
      ...
      txAuthData.functionCallName,
      (txAuthData.functionCallArgs as { owner: string }).owner,
      (txAuthData.functionCallArgs as { token_id: string }).token_id,
      txAuthData.nonce,
      txAuthData.blockExpiration,
      txAuthData.userPublicKey
    );
