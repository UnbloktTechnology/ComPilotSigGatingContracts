...
const compilotSigner = new InMemorySigner(bob_pk); // signer private key
...
describe(`GatedNftMinterSimple with SignerManagerMultisig`, function () {
	...
	let compilotSignerPublicKey: string;
	let compilotSignerAddress: string;
	...
	async function mintAsset1SignedByBob(
		...
		currentBlock: number,
		compilotSignerPublicKey: string,
	) {
	...
	beforeEach(async () => {
		...
		compilotSignerPublicKey = await compilotSigner.publicKey();
		compilotSignerAddress = await compilotSigner.publicKeyHash();
