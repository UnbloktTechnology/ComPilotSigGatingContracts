...
module Utils = struct
  ...
  let concatlists (type a) (l1: a list) (l2: a list) : a list =
    let add (acc, elt: a list * a) : a list = elt :: acc in
    List.fold add l2 l1
end
...
////////////////////////////////////////////////////////////////////////////////////////////
  ...
  let _verifyTxAuthDataWithContractAddress (type a) (p: txAuthDataWithContractAddress)(s: a siggated_storage) : a ret = 
    ...
    in
    // VERIFY - compute payload hash from parameters
    ...
    let () = if (not Utils.is_implicit(s.signerAddress)) then // case signerAddress is a smart contract
    ...
    // Emit event 
    let op_event = Tezos.emit "%SignatureVerified" (chain_id,userAddress,nonce,expiration,k,contractAddress,name, args) in
    [op_event], { s with nonces=new_nonces }


  let verifyTxAuthDataWithContractAddress (type a) (p: txAuthDataWithContractAddress)(s: a siggated_storage) : a ret = 
    _verifyTxAuthDataWithContractAddress p s   

  let verifyAndDispatchTxAuthData (type a) (p: txAuthDataWithContractAddress)(s: a siggated_storage) : a ret = 
    let { userAddress=_; expirationBlock=_expiration; contractAddress; functionName=name; functionArgs=args; signerPublicKey=_k; signature=_ } = p in
    let ops, s = _verifyTxAuthDataWithContractAddress p s in 
    // DISPATCH CALLDATA
  ...
  let verifyTxAuthData (type a) (p: txAuthData)(s: a siggated_storage) : a ret = 
    let { userAddress; expirationBlock=expiration; functionName=name; functionArgs=args; signerPublicKey=k; signature } = p in
    let p_with_address : txAuthDataWithContractAddress = { 
      userAddress; 
    ...
    _verifyTxAuthDataWithContractAddress p_with_address s 
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
  let process_internal_calldata (type a) (cd, ep: calldata * a contract) : operation =
    ...
    if (Tezos.get_self_address() = calldata_address) then
      let name = Utils.get_entrypoint_name ep in
  ...
  let process_internal_calldata_2 (type a b) (cd, ep_1, ep_2: calldata * a contract * b contract) : operation =
    let (calldata_address, calldata_name, calldata_args) = cd in
    if (Tezos.get_self_address() = calldata_address) then
      let name_1 = Utils.get_entrypoint_name ep_1 in
      if calldata_name = name_1 then  
        ...
        let name_2 = Utils.get_entrypoint_name ep_2 in
    ...
    if (Tezos.get_self_address() = calldata_address) then
      ...
      if calldata_name = name_1 then  
        ...
        if calldata_name = name_2 then  
          ...
          let name_3 = Utils.get_entrypoint_name ep_3 in
